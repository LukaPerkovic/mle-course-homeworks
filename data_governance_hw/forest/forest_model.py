...
class ActualTreeModel:
	...
	def load_data(self, path_to_data,type='csv'):
		...
		self.X_train, self.X_test, self.y_train, self.y_test = \
					 train_test_split(self.df.drop('Cover_Type',axis=1),
							  self.df.Cover_Type)
	def set_pipeline(self, scaler_type, model_type):
		...
		restof_columns =  ['Elevation', 'Aspect', 'Slope', 'Horizontal_Distance_To_Hydrology',
		...
		self.pipeline = Pipeline([
 			('column_transformer', self.col_transform),
    			('model', self.model)
			])
